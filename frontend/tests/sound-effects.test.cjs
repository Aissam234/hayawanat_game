...
const sandbox = { exports: {}, window: { AudioContext: function() { return context } },
...
sound.unlockSounds(); sound.playSound('start'); assert.equal(count,3)
sound.muteSounds(true); sound.playSound('finish'); assert.equal(count,3); assert.equal(storage.get('hayawanat_sound_muted'),'1')
sound.muteSounds(false); sound.recordingSounds(true); sound.playSound('question'); assert.equal(count,3)
sound.recordingSounds(false); playing=true; sound.playSound('turn'); assert.equal(count,3)
playing=false; sandbox.document.hidden=true; sound.playSound('wrong'); assert.equal(count,3)
sandbox.document.hidden=false; sound.playSound('finish'); assert.equal(count,7)
