...
(async()=>{
 ...
 try {
  ...
  await page.goto(process.env.TEST_URL || 'http://127.0.0.1:15173');
  ...
  assert.equal(await page.locator('input[name=avatar]:checked').count(),0);
  assert.equal(await page.locator('form').evaluate(form=>form.checkValidity()),false);
  await page.getByRole('radio',{name:'ثعلب',exact:true}).check();
  ...
  await page.getByRole('img',{name:'الصورة الشخصية: ثعلب'}).waitFor();
  await page.reload();
  await page.getByText(username,{exact:true}).waitFor();
  await page.getByRole('img',{name:'الصورة الشخصية: ثعلب'}).waitFor();
